...
* G(V, E), A是V的一一个子子集, 求至至少包含A中所有点的最小小子子树.
* 时间复杂度:O(N^3+N*2^A*(2^A+N))
* INIT: d[][]距离矩阵; id[]置为集合A中点的标号;
* CALL: steiner(int n, int a);
* 给4个点对(a1,b1)...(a4,b4),
* 求min(sigma(dist[ai][bi])),其中重复的路路段只能算一一次.
* 这题要找出一一个Steiner森林林, 最后要对森林林中树的个数进行行行枚举
*/
#define typec int
// type of cost
const typec inf = 0x3f3f3f3f;
const typec V = 10010;
const typec A = 10; // max of cost
int vis[V], id[A];
typec d[V][V], dp[1 << A][V]; // id[]:
A中点的标号
// dp[i][v]: 点v到点集i的最短距离
